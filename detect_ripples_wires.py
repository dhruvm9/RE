...
file = os.path.join(data_directory, 'B0801-211118.sws.evt')
if os.path.exists(file):
    tmp = np.genfromtxt(file)[:,0]
    tmp = tmp.reshape(len(tmp)//2,2)/1000
    sws_ep = nts.IntervalSet(start = tmp[:,0], end = tmp[:,1], time_units = 's')
...
lfp = loadLFP(os.path.join(data_directory,data_directory.split('/')[-1]+'.eeg'), n_channels, 0, 1250, 'int16')
