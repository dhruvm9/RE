...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 23 15:51:48 2021
...
from wrappers import loadPos_Adrian
...
data_directory = '/media/DataDhruv/Recordings/GRV_test'
...
spikes = pickle.load(open(data_directory + '/spikes.pickle', 'rb'))
...
for i, n in enumerate(tuning_curves.columns):
    ...
    subplot(8,3,i+1, projection = 'polar')
...
#ac = autocorr[0][autocorr[0].index.values >0]
#ac = ac.rolling(window = 20, win_type = 'gaussian', center = True, min_periods = 1).mean(std = 3.0)
...
# spatial_curves1, extent = computePlaceFields(spikes, position[['x', 'z']], wake2_ep.loc[[0]], 40)
# spatial_curves2, extent = computePlaceFields(spikes, position[['x', 'z']], wake2_ep.loc[[1]], 40)
