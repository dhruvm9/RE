...
def makePositions(path, file_order, episodes, n_ttl_channels = 1, optitrack_ch = None, names = ['ry', 'rx', 'rz', 'x', 'y', 'z'], update_wake_epoch = True):
    ...
    for i, f in enumerate(file_order):
        ...
        if not os.path.split(analogin_file)[1] in files:
            ...
            if i == 0:
                ttl = loadTTLPulse(analogin_file) # sandy
            elif i == 1:
                ttl = loadTTLPulse(analogin_file, 2, 1) # adrian
        ...
        #######################
        # FOR MERGING ADRIAN'S CSV
        #######################
        col_names = []
        for c in position.columns:
            if c[0] == 'Rotation':
                col_names.append('r'+ c[1].lower())
            elif c[0] == 'Position':
                col_names.append(c[1].lower())
        
        position.columns = col_names
...
def loadPos_Adrian(path, events = None, episodes = None, n_ttl_channels = 1, optitrack_ch = None, names = ['rx', 'rz', 'ry', 'x', 'y', 'z'], update_wake_epoch = True):
    """
    load the position contained in /Analysis/Position.h5
    Notes:
        The order of the columns is assumed to be
